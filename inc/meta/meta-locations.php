...
add_action(
	...
	function (): void {
		add_meta_box(
			'drumstudy_location_meta',
			__( 'Location Details', 'drumstudy' ),
			'drumstudy_location_meta_cb',
			'drumstudy_location',
			'normal',
			'high'
		);
 ...
 */
function drumstudy_location_meta_cb( WP_Post $post ): void {
	wp_nonce_field( 'drumstudy_save_location_meta', 'drumstudy_location_nonce' );
	...
	<div class="tts-meta-grid">
		<div class="tts-meta-row">
			<label for="address_1" class="tts-meta-label"><?php esc_html_e( 'Address Line 1', 'drumstudy' ); ?> <span class="required">*</span></label>
			<input type="text" id="address_1" name="address_1" value="<?php echo esc_attr( $address_1 ); ?>" class="widefat" />
			<p class="description"><?php esc_html_e( 'Where it appears: Location card and single page. Required.', 'drumstudy' ); ?></p>
		</div>
		<div class="tts-meta-row">
			<label for="address_2" class="tts-meta-label"><?php esc_html_e( 'Address Line 2', 'drumstudy' ); ?></label>
			<input type="text" id="address_2" name="address_2" value="<?php echo esc_attr( $address_2 ); ?>" class="widefat" />
		</div>
		<div class="tts-meta-row">
			<label for="city" class="tts-meta-label"><?php esc_html_e( 'City', 'drumstudy' ); ?></label>
			<input type="text" id="city" name="city" value="<?php echo esc_attr( $city ); ?>" class="widefat" />
		</div>
		<div class="tts-meta-row">
			<label for="state" class="tts-meta-label"><?php esc_html_e( 'State / Province', 'drumstudy' ); ?></label>
			<input type="text" id="state" name="state" value="<?php echo esc_attr( $state ); ?>" class="widefat" />
		</div>
		<div class="tts-meta-row">
			<label for="postal" class="tts-meta-label"><?php esc_html_e( 'Postal Code', 'drumstudy' ); ?></label>
			<input type="text" id="postal" name="postal" value="<?php echo esc_attr( $postal ); ?>" class="widefat" />
		</div>
		<div class="tts-meta-row">
			<label for="location_phone" class="tts-meta-label"><?php esc_html_e( 'Phone', 'drumstudy' ); ?></label>
			<input type="text" id="location_phone" name="location_phone" value="<?php echo esc_attr( $location_phone ); ?>" class="widefat" />
		</div>
		<div class="tts-meta-row">
			<label for="location_email" class="tts-meta-label"><?php esc_html_e( 'Email', 'drumstudy' ); ?></label>
			<input type="email" id="location_email" name="location_email" value="<?php echo esc_attr( $location_email ); ?>" class="widefat" />
		</div>
		<div class="tts-meta-row">
			<label for="manager_name" class="tts-meta-label"><?php esc_html_e( 'Manager Name', 'drumstudy' ); ?></label>
			<input type="text" id="manager_name" name="manager_name" value="<?php echo esc_attr( $manager_name ); ?>" class="widefat" />
		</div>
		<div class="tts-meta-row">
			<label for="map_embed" class="tts-meta-label"><?php esc_html_e( 'Google Maps Embed URL', 'drumstudy' ); ?></label>
			<input type="url" id="map_embed" name="map_embed" value="<?php echo esc_attr( $map_embed ); ?>" class="widefat" />
			<p class="description"><?php esc_html_e( 'Where it appears: Embedded map on location single page. Paste URL from Google Maps → Share → Embed.', 'drumstudy' ); ?></p>
		</div>
		<div class="tts-meta-row tts-meta-row--full">
			<label for="location_hours" class="tts-meta-label"><?php esc_html_e( 'Hours', 'drumstudy' ); ?></label>
			<textarea id="location_hours" name="location_hours" rows="5" class="widefat"><?php echo esc_textarea( $location_hours ); ?></textarea>
			<p class="description"><?php esc_html_e( 'Where it appears: Location card and single page. One line per day.', 'drumstudy' ); ?></p>
		</div>
		<div class="tts-meta-row">
			<label class="tts-meta-label"><?php esc_html_e( 'Location Image', 'drumstudy' ); ?></label>
			...
			<button type="button" class="button tts-media-upload-btn" data-field="location_image" data-preview="location_image_preview"><?php esc_html_e( 'Select Image', 'drumstudy' ); ?></button>
			<?php if ( $img_id ) : ?>
				<button type="button" class="button tts-media-remove-btn" data-field="location_image" data-preview="location_image_preview"><?php esc_html_e( 'Remove', 'drumstudy' ); ?></button>
			<?php endif; ?>
		</div>
	</div>
	<?php
 ...
 */
function drumstudy_save_location_meta( int $post_id ): void {
	if ( ! isset( $_POST['drumstudy_location_nonce'] )
		|| ! wp_verify_nonce( sanitize_key( $_POST['drumstudy_location_nonce'] ), 'drumstudy_save_location_meta' ) ) {
		return;
	}
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
	if ( ! current_user_can( 'edit_post', $post_id ) ) return;
	if ( 'drumstudy_location' !== get_post_type( $post_id ) ) return;
...
add_action( 'save_post', 'drumstudy_save_location_meta' );

add_filter(
	'drumstudy_image_meta_keys',
	function ( array $keys, int $post_id ): array {
		if ( 'drumstudy_location' === get_post_type( $post_id ) ) {
			$keys[] = 'location_image';
		}
		return $keys;
	},

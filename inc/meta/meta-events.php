 ...
 */
function tts_event_meta_cb( WP_Post $post ): void {
	wp_nonce_field( 'tts_save_event_meta', 'tts_event_nonce' );
	...
	<div class="tts-meta-grid">
		<div class="tts-meta-row">
			<label for="event_date" class="tts-meta-label"><?php esc_html_e( 'Event Date', 'tts-theme' ); ?> <span class="required">*</span></label>
			<input type="date" id="event_date" name="event_date" value="<?php echo esc_attr( $event_date ); ?>" class="widefat" />
			<p class="description"><?php esc_html_e( 'Where it appears: Event card and archive. Required — controls upcoming/past sorting.', 'tts-theme' ); ?></p>
		</div>
		<div class="tts-meta-row">
			<label for="event_time" class="tts-meta-label"><?php esc_html_e( 'Start Time', 'tts-theme' ); ?></label>
			<input type="text" id="event_time" name="event_time" value="<?php echo esc_attr( $event_time ); ?>" class="widefat" placeholder="e.g. 7:00 PM" />
		</div>
		<div class="tts-meta-row">
			<label for="end_date" class="tts-meta-label"><?php esc_html_e( 'End Date', 'tts-theme' ); ?></label>
			<input type="date" id="end_date" name="end_date" value="<?php echo esc_attr( $end_date ); ?>" class="widefat" />
			<p class="description"><?php esc_html_e( 'Leave blank for single-day events.', 'tts-theme' ); ?></p>
		</div>
		<div class="tts-meta-row">
			<label for="end_time" class="tts-meta-label"><?php esc_html_e( 'End Time', 'tts-theme' ); ?></label>
			<input type="text" id="end_time" name="end_time" value="<?php echo esc_attr( $end_time ); ?>" class="widefat" placeholder="e.g. 10:00 PM" />
		</div>
		<div class="tts-meta-row">
			<label for="location_name" class="tts-meta-label"><?php esc_html_e( 'Venue Name', 'tts-theme' ); ?></label>
			<input type="text" id="location_name" name="location_name" value="<?php echo esc_attr( $location_name ); ?>" class="widefat" />
			<p class="description"><?php esc_html_e( 'Where it appears: Event card and single page.', 'tts-theme' ); ?></p>
		</div>
		<div class="tts-meta-row">
			<label for="location_address" class="tts-meta-label"><?php esc_html_e( 'Venue Address', 'tts-theme' ); ?></label>
			<input type="text" id="location_address" name="location_address" value="<?php echo esc_attr( $location_address ); ?>" class="widefat" />
		</div>
		<div class="tts-meta-row">
			<label for="ticket_url" class="tts-meta-label"><?php esc_html_e( 'Ticket URL', 'tts-theme' ); ?></label>
			<input type="text" id="ticket_url" name="ticket_url" value="<?php echo esc_attr( $ticket_url ); ?>" class="widefat" />
			<p class="description"><?php esc_html_e( 'Where it appears: Get tickets CTA on event card and single page.', 'tts-theme' ); ?></p>
		</div>
		<div class="tts-meta-row">
			<label for="ticket_price" class="tts-meta-label"><?php esc_html_e( 'Ticket Price', 'tts-theme' ); ?></label>
		...
		<div class="tts-meta-row">
			<label for="organizer" class="tts-meta-label"><?php esc_html_e( 'Organizer', 'tts-theme' ); ?></label>
			<input type="text" id="organizer" name="organizer" value="<?php echo esc_attr( $organizer ); ?>" class="widefat" />
		</div>
		<div class="tts-meta-row">
			<label class="tts-meta-label"><?php esc_html_e( 'Event Image', 'tts-theme' ); ?></label>
			...
			<button type="button" class="button tts-media-upload-btn" data-field="event_image" data-preview="event_image_preview"><?php esc_html_e( 'Select Image', 'tts-theme' ); ?></button>
			<?php if ( $img_id ) : ?>
				<button type="button" class="button tts-media-remove-btn" data-field="event_image" data-preview="event_image_preview"><?php esc_html_e( 'Remove', 'tts-theme' ); ?></button>
			<?php endif; ?>
		</div>
		<div class="tts-meta-row tts-meta-row--full">
			<label for="embed_block" class="tts-meta-label"><?php esc_html_e( 'Embed / Shortcode Block', 'tts-theme' ); ?></label>
			<textarea id="embed_block" name="embed_block" rows="4" class="widefat"><?php echo esc_textarea( $embed_block ); ?></textarea>
			<p class="description"><?php esc_html_e( 'Where it appears: Below event description. Accepts shortcodes, embed URLs, iframe codes.', 'tts-theme' ); ?></p>
		</div>
	</div>
	<?php
}
 ...
 */
function tts_save_event_meta( int $post_id ): void {
	if ( ! isset( $_POST['tts_event_nonce'] )
		|| ! wp_verify_nonce( sanitize_key( $_POST['tts_event_nonce'] ), 'tts_save_event_meta' ) ) {
		return;
	}
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
	if ( ! current_user_can( 'edit_post', $post_id ) ) return;
	if ( 'tts_event' !== get_post_type( $post_id ) ) return;
...
add_action( 'save_post', 'tts_save_event_meta' );

add_filter(
	'tts_image_meta_keys',
	function ( array $keys, int $post_id ): array {
		if ( 'tts_event' === get_post_type( $post_id ) ) {
			$keys[] = 'event_image';
		}
		return $keys;
	},
